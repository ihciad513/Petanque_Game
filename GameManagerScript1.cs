...
public class GameManagerScript1 : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (secondShot == true)
		{
			if (Prb2.IsSleeping())
			{
				...
				if (BPD1 > BPD2 == true)
				{
					...
					if (enemycount == 2 && Erb2.IsSleeping())
					{
						if (Player > Enemy)
						{
		...
		if (thirdShot == true)
		{
			if (Prb3.IsSleeping())
			{
				...
				if (Player > BPD3 == true)
				{
                    ...
                    if(Player < BED1 == true && enemycount==1)
                    {
					...
					if (count == 3 && enemycount == 2 && thirdShot == true)//PEPEP→E
					{
				...
				if (count == 3 && enemycount == 3 && Erb3.IsSleeping())//双方投げ切り
				{
					if (Player > Enemy)
					{
		...
		if (thirdShotEnemy == true)
		{
			...
			if (Erb3.IsSleeping())
			{
				...
				if (count == 1 && enemycount==3)
				{
					
					Trueplayer = true;//PEEE→P
					FifthTurn();
				}
				if (count == 2 && enemycount==3)
				{
					
					Trueplayer = true;//PEEEP→P
					SixthTurn();
					if (count == 3 && Prb3.IsSleeping())
					{
						ScoreCount();
						PlayerPrefs.SetInt("PS", scorecount);
						SceneManager.LoadScene("WinScene");
				...
				if (count == 3 && enemycount == 3)//双方投げ切り
				{
					if (Player > Enemy)
					{
	...
	void FourthTurn()
	{
		...
		count += 1;
			if (count == 1)
			{
				if (shotbutton == true)
				{
					...
					player1 = Instantiate(Ball1, this.transform.position+this.transform.forward, Quaternion.Euler(new Vector3(0f, 45, 0f))) as GameObject;
			...
			if (shotbutton == true)
				...
				player2 = Instantiate(Ball1, this.transform.position, Quaternion.Euler(new Vector3(0f,45,0f))) as GameObject;
			...
			if (count == 3)
			{
				...
				player3 = Instantiate(Ball1, this.transform.position, Quaternion.Euler(new Vector3(0f, 45, 0f))) as GameObject;
		...
		private void ThrowingBall()
		{
			...
			if (enemycount == 1 && Erb1.IsSleeping())
			{
				enemycount += 1;
				float y = Random.Range(3.0f, 15.0f);
				float z = Random.Range(3.0f, 15.0f);
